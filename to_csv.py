...
TITLE = "2026 Best Objects (Adin, 11 Jul 2026)"
...
def main():
    ...
    for obj in objects:
        ...
        rh, rm, dd, dm = radec_fields(ra, dec)
        ...
        n_field = "%s %s %s" % (label, descriptor, fmt_mag(mag))
        rows.append("%s,%d,%s,%s,%s" % (n_field, rh, rm, dd, dm))
        if tword in ("Deep Sky",) or mag is None:
    ...
    print("Wrote %s  (%d objects)" % (DST, len(rows)))
    if notes:
